...
from . import BRAIN_CHECKER, LOGS, bot, PLUGIN_CHANNEL_ID, CMD_HELP, LANGUAGE, GOLD_VERSION, PATTERNS
...
try:
    bot.start()
    idim = bot.get_me().id
    goldbl = requests.get('https://gitlab.com/Quiec/asen/-/raw/master/asen.json').json()
    if idim in goldbl:
        bot.disconnect()
    ...
    PLUGIN_MESAJLAR = {}
    ORJ_PLUGIN_MESAJLAR = {"alive": "`ALLAH Azeri'ni Qorusun. 🐺 GoldUserBot İşləyir.`", "afk": f"`{str(choice(AFKSTR))}`", "kickme": "`sağol mən gedirəm `🤠", "pm": UNAPPROVED_MSG, "dızcı": str(choice(DIZCILIK_STR)), "ban": "{mention}`, qadağan edildi!`", "mute": "{mention}`, səssizə alındı!`", "approve": "{mention}`, mənə mesaj göndərə bilərsiniz!`", "disapprove": "{mention}`, daha mənə mesaj göndərə bilməzsən!`", "block": "{mention}`, siz bloklanmısınız!`"}
...
LOGS.info("Botunuz işləyir! İstənilən söhbətdə .alive yazaraq onu yoxlayın."
           "Köməyə ehtiyacınız varsa, Dəstək qrupumuza gəlin t.me/GoldSupport")
LOGS.info(f"Bot sürümünüz: Gold {GOLD_VERSION}")
