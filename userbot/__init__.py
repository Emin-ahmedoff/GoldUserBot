...
if CONSOLE_LOGGER_VERBOSE:
    basicConfig(
        format="%(asctime)s - @GoldUserBot - %(levelname)s - %(message)s",
        level=DEBUG,
    )
else:
    basicConfig(format="%(asctime)s - @GoldUserBot - %(levelname)s - %(message)s",
                level=INFO)
...
# SQL Veritabanı
DB_URI = os.environ.get("DATABASE_URL", "sqlite:///gold.db")
...
# Last.fm Modülü
BIO_PREFIX = os.environ.get("BIO_PREFIX", "@GoldUserBot | ")
...
PAKET_ISMI = os.environ.get("PAKET_ISMI", "@GoldUserBot Paketi")
...
with bot:
    if OTOMATIK_KATILMA:
        try:
            bot(JoinChannelRequest("@GoldUserBot"))
            bot(JoinChannelRequest("@GoldUserBotplugin"))
        except:
            pass
    ...
    try:
        ...
        async def start_bot_handler(event):
            if not event.message.from_id == uid:
                await event.reply(f'`Salam mən` @GoldUserBot`! Mən sahibimə (`@{me.username}`) kömək etmək üçün varam, yəni sənə kömək edə bilmərəm :/ Ama sen de bir GoldUserBot açabilərsən; Qrupa bax` @GoldUserBot')
            else:
                await event.reply(f'`Tengri save Azeri! GoldUserBot working... 🐺`')
        ...
        async def inline_handler(event):
            ...
            result = None
            query = event.text
            if event.query.user_id == uid and query == "@GoldUserBot":
                rev_text = query[::-1]
                veriler = (butonlastir(0, sorted(CMD_HELP)))
                result = await builder.article(
                    f"Lütfen Sadece .yardım Komutu İle Kullanın",
                    text=f"**♣️ GoldUserBot!** [Gold](https://t.me/GoldUserBot) __İşləyir...__\n\n**Yüklənənn Modül Sayı:** `{len(CMD_HELP)}`\n**Səyfə:** 1/{veriler[0]}",
                    buttons=veriler[1],
                    link_preview=False
                )
                ...
                result = builder.article(
                    "@GoldUserBot",
                    text="""@GoldUserBot'u işəltməyi yoxlayin!
Hesabınızı bot'a çevirə bilirsiniz və bunları istifadə edə bilərsiniz. Unutmayın, siz başqasının botunu yönetemezsiniz! Alttaki GitHub adresindən bütün quraşdırma detalları izah edilmişdir.""",
                    buttons=[
                        [custom.Button.url("Kanala Katıl", "https://t.me/GoldUserBot"), custom.Button.url(
                            "Gruba Katıl", "https://t.me/GoldUserBot")],
                        [custom.Button.url(
                            "GitHub", "https://github.com/Emin-ahmedoff/GoldUserBot")]
                    ],
                    link_preview=False
                )
        ...
        async def sayfa(event):
            if not event.query.user_id == uid: 
                return await event.answer("❌ hey! Mesajlarımı redaktə etməyə çalışmayın! Özünə bir @GoldUserBot qur.", cache_time=0, alert=True)
            sayfa = int(event.data_match.group(1).decode("UTF-8"))
            veriler = butonlastir(sayfa, CMD_HELP)
            await event.edit(
                f"**🐺 Allah Azerini qorusun!** [gold](https://t.me/GoldUserBot) __İşləyir...__\n\n**Yüklənən Modul Sayı:** `{len(CMD_HELP)}`\n**Səyfə:** {sayfa + 1}/{veriler[0]}",
                buttons=veriler[1],
                link_preview=False
            )
